...
import type { GameResult, GameMode, InputEvent } from './types';
...
export function useGameSession() {
  ...
  const createSession = useCallback(async (mode: GameMode = 'normal', challengeId?: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/games/tap-rush/session`, {
        method: 'POST',
        headers: {
  ...
  const submitResult = useCallback(
    async (events: InputEvent[], durationMs: number): Promise<GameSessionResult | null> => {
      if (!state.sessionId) {
      ...
      try {
        const response = await fetch(
          `${API_BASE}/api/games/tap-rush/session/${state.sessionId}/complete`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            ...
            body: JSON.stringify({
              events: events.map((e) => ({
                ...
                tapX: e.tapPosition?.x,
                tapY: e.tapPosition?.y,
                combo: e.combo,
                data: e.data,
              })),
              durationMs,
